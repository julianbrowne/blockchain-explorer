...
function BlockchainExplorer() { 
	...
	this.chainOfHashes = function(numberOfInputs) { 
		...
		for(let i=0; i<numberOfInputs; i++) { 
			...
			const label = arrow + " Digest " + itemNumber;
	...
	this.mining = function() { 
		...
		**/
		...
		for(let i=0; i<config.minerDataInputs; i++) { 
			...
			inputCombo.find("input").eq(0).keyup(updateHashes);

			formGroup.append(inputCombo);

			const hash = inputCombo.find("input").eq(1).val();
		...
		**/
		...
		nonceInputGroup.find("input").keyup(updateHashes);
		...
		**/
		...
		topHashInputGroup.find("input").keyup(function(event) { 
